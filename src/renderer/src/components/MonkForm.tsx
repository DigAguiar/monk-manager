...
export function MonkForm({ initialData, onSubmit, onCancel }: MonkFormProps) {
  ...
  // Helper para Inputs de Data com Máscara
  const DateInput = ({
    name,
    label,
    placeholder
    ...
    <div>
      ...
      <input
        ...
        onChange={(e) => {
          // Aplica a máscara e atualiza o react-hook-form manualmente
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold mb-6 text-gray-800">
        {initialData ? 'Editar Monge' : 'Novo Monge'}
      </h2>
      ...
      <div className="flex border-b mb-6">
        {tabs.map((tab, index) => (
          <button
            ...
            className={`px-6 py-2 font-medium outline-none transition-colors ${
              activeTab === index
      ...
      <div className="min-h-[400px]">
        ...
        {activeTab === 0 && (
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* MUDANÇA AQUI: Agora usamos o DateInput com máscara */}
        ...
        {activeTab === 1 && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium">Título Religioso</label>
              <input
                {...register('titulo')}
                placeholder="Ex: Frei, Dom, Abade"
                className="w-full p-2 border rounded"
              />
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            <div className="col-span-2">
              ...
              <input
                {...register('local_profissao_religiosa')}
                className="w-full p-2 border rounded"
              />
            </div>
            <div className="col-span-2">
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium">Ocupação / Ofícios</label>
              <input
                {...register('ocupacao_oficio')}
                placeholder="Ex: Bibliotecário, Porteiro"
                className="w-full p-2 border rounded"
              />
            </div>
            <div className="col-span-2">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="border p-4 rounded bg-gray-50">
              ...
              <div className="flex gap-2 mb-2">
                <input
                  value={livroInput}
                  onChange={(e) => setLivroInput(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && handleAddLivro(e)}
                  placeholder="Digite o nome do livro e aperte Enter"
              ...
              <ul className="space-y-1 max-h-32 overflow-y-auto">
                ...
                {fields.length === 0 && (
                  <p className="text-gray-400 text-sm italic">Nenhum livro cadastrado.</p>
                )}
              </ul>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium">Efemérides</label>
                <input
                  {...register('episodios_efemerides')}
                  className="w-full p-2 border rounded"
                />
              </div>
              <div>
                <label className="block text-sm font-medium">Exercícios Espirituais</label>
                <input
                  {...register('exercicios_espirituais')}
                  className="w-full p-2 border rounded"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium">Referência Manuscrito</label>
              <input {...register('referencia_manuscrito')} className="w-full p-2 border rounded" />
            </div>
            <div>
              <label className="block text-sm font-medium">Referência Edição</label>
              <input {...register('referencia_edicao')} className="w-full p-2 border rounded" />
            </div>
          </div>
        )}
      </div>
